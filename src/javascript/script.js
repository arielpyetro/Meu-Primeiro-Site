window.onload = () => {
  const translations = {
    pt: {
      typing: "<Olá Mundo/>",
      subtitle: "Não sou programador, faço tudo por hobby."
    },
    es: {
      typing: "<Hola Mundo/>",
      subtitle: "No soy programador, hago todo por hobby."
    }
  };

  let lang = localStorage.getItem('lang');
  if (!lang) {
    lang = (navigator.language || navigator.userLanguage).startsWith('es') ? 'es' : 'pt';
  ...
  let text = translations[lang].typing;
  let index = 0;
  ...
  subtitleElement.innerText = translations[lang].subtitle;
